...
public abstract class DbH2 extends DbDefinitionBase implements HandicapDatabaseG {
  ...
  protected final static String CHECKHANDICAPSQL = "SELECT table_name FROM INFORMATION_SCHEMA.TABLES WHERE table_type='BASE TABLE' AND table_name in ('golfer', 'course', 'scores', 'ratings', 'groups', 'member', 'login')";
  ...
  protected enum CreateTable {
    ...
    CREATELOGIN("create table IF NOT EXISTS login (id int auto_increment NOT NULL PRIMARY KEY, name varchar(255) not null unique, password varchar(255) not null, last_login TIMESTAMP not null)"),
