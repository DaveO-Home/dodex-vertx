...
public class DodexDatabaseNeo4j extends DbNeo4j {
  ...
  public Promise<Driver> databaseSetup() {
    ...
    session.runAsync(getCheckConstraints()).thenCompose(fn -> fn.singleAsync())
        .handle((record, exception) -> {
          ...
          if (count < 4) {
            session.runAsync(getCheckApoc()).thenCompose(fn -> fn.singleAsync())
  ...
  private void apocConstraints(Driver driver, Promise<Driver> promise) {
    Multi.createFrom().resource(driver::rxSession, session -> session.writeTransaction(tx -> {
      RxResult resultConstraints = tx.run(getCreateConstraints());
      return Multi.createFrom().publisher(resultConstraints.records()).map(record -> "constraints");
    })).withFinalizer(session -> {
      promise.complete(driver);
      return Uni.createFrom().publisher(session.close());
    }).onFailure().invoke(Throwable::printStackTrace).subscribe().asStream();
  }
  ...
  private Driver getDriver(Map<String, String> dbMap, Properties dbProperties) {
    String uri = String.join(":", dbMap.get("protocol"), String.format("//%s", dbMap.get("host")),
        dbMap.get("port"));
