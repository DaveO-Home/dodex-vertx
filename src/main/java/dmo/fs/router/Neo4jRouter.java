...
public class Neo4jRouter {
  ...
  public void setWebSocket(final HttpServer server)
    ...
    CompletableFuture<Driver> cs = dbPromise.future().onItem().call(driver -> {
      setDriver(driver);
    ...
    server.webSocketHandler(ws -> {
      ...
      if (!"/dodex".equals(ws.path())) {
        ...
        query = ParseQueryUtilHelper
            .getQueryMap((wsChatSessions.get(ws.remoteAddress().toString())).toString());
        ...
        try {
          ...
          future.future().onItem().call(messageUser2 -> {
            try {
              ...
              CompletableFuture<StringBuilder> uj = userJson.future().onItem().call(json -> {
                ws.writeTextMessage("connected:" + json);
                /*
                 * Send undelivered messages and remove user related messages.
                 */
                try {
                  CompletableFuture<Map<String, Integer>> pum = dodexNeo4j.processUserMessages(ws, messageUser2).future()
                      .onItem().call(counts -> {
                        final int messageCount = counts.get("messages");
                        if (messageCount > 0) {
                          logger.info("{}Messages Delivered: {} to {}{}", ColorUtilConstants.BLUE_BOLD_BRIGHT, messageCount, messageUser.getName(), ColorUtilConstants.RESET);
                          if (ke != null) {
              ...
              for (final String websocket : websockets) {
                ...
                if (!webSocket.isClosed()) {
                  if (!websocket.equals(ws.remoteAddress().toString())) {
                    // broadcast message
                    query = ParseQueryUtilHelper
                        .getQueryMap(wsChatSessions.get(webSocket.remoteAddress().toString()).toString());
